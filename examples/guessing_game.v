...
(** To illustrate the some non-determinism I'm trying to specify the
  guessing game from the rust book, the idea is to start with the
  spec and refine it down to a C program *)

(** * Effect signatures *)

Inductive game_sig : esig :=
  | pick : game_sig unit
  | guess (n : nat) : game_sig comparison.
...
Definition game_state : Type :=
  nat.

Search (nat -> nat -> comparison).

Definition L_game : CALspec game_sig game_state :=
  fun ar m n =>
    match m with
      | pick =>
...
Goal
  ...
  forall f,
    ...
    ISpec.ret (f 2, (f, 3, 5)).
Proof.
  intros H f. unfold layer.
  rewrite <- ISpec.apply_assoc. intm.
  rewrite assert_true by lia.
